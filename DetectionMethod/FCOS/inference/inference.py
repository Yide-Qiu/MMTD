...
def detections_from_net(boxes_by_batch, classes_by_batch, scores_by_batch=None) -> List[List[Detection]]:
    ...
    for batch in range(len(classes_by_batch)):
        ...
        for i in range(boxes.shape[0]):
            if classes_np[i] != 0:
                # pdb.set_trace()
                if boxes_np[i][0] <0 or boxes_np[i][1] <0 or boxes_np[i][2] >=1024 or boxes_np[i][3] >=1024:
                    continue
                result.append(
                    [
                        Detection(
...
def _gather_detections(classes, centernesses, boxes, max_detections=DEFAULT_MAX_DETECTIONS):
    ...
    # detection_scores = classes.squeeze(-1) * centernesses
    class_scores, class_indices = torch.max(classes, dim=2)
    # pdb.set_trace()
    class_indices = torch.zeros(size=classes.shape).to(classes.device)
    class_indices[classes > 0.5] = 1
