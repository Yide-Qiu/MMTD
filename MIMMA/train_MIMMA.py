...
def evaluate_loss(model, dataloader, config):
    ...
    with torch.no_grad():
        for i in range(len(dataloader)):
            ...
            norms = torch.norm(out_asso, p=2, dim=1, keepdim=True) 
            normalized_matrix = out_asso / norms  
            ...
            TP = torch.sum((association_map == 1) & equal_mask) 
            FP = torch.sum((association_map == 1) & ~equal_mask) 
            TN = torch.sum((association_map == 0) & equal_mask)  
            FN = torch.sum((association_map == 0) & ~equal_mask) 
            asso_cm[0] += TP
            asso_cm[1] += FP
            asso_cm[2] += TN
            asso_cm[3] += FN
