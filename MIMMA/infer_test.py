...
def init_config():
    ...
    # 用于组件消融
    parser.add_argument("--module", type=str, default="MIMMA", help="['MFTP','MSTA','MIMMA']")

    # 用于 probability thresh 消融
    parser.add_argument("--thresh_prob_ablation", type=int, default=0, help="[0, 1]")
    parser.add_argument("--thresh_prob", type=float, default=0.5, help="[0, 1]")

    # 用于 ground thresh 消融
    parser.add_argument("--thresh_ground_ablation", type=int, default=0, help="[0, 1]")
    parser.add_argument("--thresh_ground", type=float, default=0.04, help="[0, 1]")
...
def run_prob_thresh_ablation(args):
    ...
    thresh_prob_list = range(1, 10, 1)
    args.thresh_ground = 0.04
    for thresh_prob in thresh_prob_list:
        args.thresh_prob = thresh_prob / 10
        args.exp_name = args.dataset_name + '_' + args.module + '_prob_thresh_' + str(thresh_prob / 10)
        for file_name in os.listdir(obs_path_split):
            # file_name = 'area_1.txt'
...
def run_ground_thresh_ablation(args):
    data_root = args.data_root
    # file_name = args.file_name
    row_obs_dataset_name = args.dataset_name + '_obs'
    row_gt_dataset_name = args.dataset_name + '_gt'
    ...
    print(f'load obs data from {obs_path_split}')
    print(f'load gt data from {gt_path_split}')
    thresh_ground_list = range(1, 10, 1)
    args.thresh_prob = 0.9
    for thresh_ground in thresh_ground_list:
        args.thresh_ground = thresh_ground / 100
        args.exp_name = args.dataset_name + '_' + args.module + '_ground_thresh_' + str(thresh_ground / 100)
        for file_name in os.listdir(obs_path_split):
            # file_name = 'area_1.txt'
            args.file_name = file_name
            print(file_name)
...
if __name__ == "__main__":
    args = init_config()
    if args.thresh_prob_ablation:
        run_prob_thresh_ablation(args)
    elif args.thresh_ground_ablation:
        run_ground_thresh_ablation(args)
    else:
        run_normal(args)
