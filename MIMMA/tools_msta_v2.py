...
class MultiSensorShipTrackingModel:
    ...
    def long_traj_update(self, obs_array_t, traj_pred_result_points, ship_mmsi_long, ship_traj_long):
        ...
        for pred_idx, obs_idx in matches:
            ...
            if obs_array_t[obs_idx][-1] in ['radar']:
                ...
                self.ship_traj_dict_pred[ship_mmsi_long[pred_idx]]['traj'].append(np.array([obs_array_t[0, 0], traj_pred_result_points[pred_idx][0] - h,
    ...
    def short_traj_update(self, obs_array_t, traj_pred_points_short, ship_traj_short, ship_mmsi_short, matched_obs_idx_list):
        ...
        matches, cost_matrix = hungarian_matching_traj(traj_pred_points_short, obs_array_t[:, 3:5].astype(np.float32))  # 选用第t+1帧的预测结果
        # pdb.set_trace()
        for pred_idx, obs_idx in matches:
            # 对于匹配上的{观测点,预测点}，将预测点加入到ship_traj_list中
            if cost_matrix[pred_idx, obs_idx] >= self.association_short_thresh:
                continue
            # if obs_idx in matched_obs_idx_list:
            #     continue
            # import pdb
            ...
            for t1 in range(int(ship_traj_short[pred_idx, 0]) + 1, crt_t):
                ...
                # self.ship_traj_dict_pred[ship_mmsi_short[pred_idx]]['traj'][-1][-2] = 'pred'
        ...
        for i in range(len(ship_mmsi_short)):
            if i not in matched_pred_idx_list:
                ...
                if self.ship_traj_break_time_dict[ship_mmsi_short[i]] >= self.finish_time_max:
                    self.ship_traj_finish_dict[ship_mmsi_short[i]] = self.ship_traj_dict_pred[ship_mmsi_short[i]]
    ...
    def add_new_traj(self, obs_array_t, matched_obs_idx_list):
        ...
        for i in range(obs_array_t.shape[0]):
            if i not in matched_obs_idx_list:
                ...
                self.pred_mmsi += 1
                # array([[0, 42.36045567357177, -179.39510706250783, 42.36075211625952,-179.3974655348367,
                # -1.0, -1.0, 'elec', 'satellite_3053653107']],dtype=object)
                # pdb.set_trace()
                tlbr = obs_array_t[i][1:5]
                track = STrack(STrack.tlbr_to_tlwh(tlbr), 1)
                track.activate(self.kalman_filter, self.frame_id)
                self.short_stracks.append(track)
    ...
    def multi_object_tracking(self, obs_array_t):
        ...
        # if self.frame_id % self.merge_t_interval == 0:
        #     # 执行融合
        #     # pdb.set_trace()
    ...
    def get_ship_traj_dict(self):
        ...
        for traj_id in list(self.ship_traj_dict_pred.keys()):
            ...
            if len(traj) < self.finish_time_max:
