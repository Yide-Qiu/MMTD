...
def evaluate_loss(model, dataloader, config):
    ...
    with torch.no_grad():
        for i in range(len(dataloader)):
            ...
            TP = np.sum((association_map == 1) & equal_mask)  
            FP = np.sum((association_map == 1) & ~equal_mask)
            TN = np.sum((association_map == 0) & equal_mask) 
            FN = np.sum((association_map == 0) & ~equal_mask)
            asso_cm[0] += TP
            asso_cm[1] += FP
            asso_cm[2] += TN
            asso_cm[3] += FN
