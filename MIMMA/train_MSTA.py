...
def evaluate_loss(model, dataloader, config):
    ...
    with torch.no_grad():
        for i in range(len(dataloader)):
            ...
            # pdb.set_trace()
            gt_asso_map = mmsi_matrix.cpu().numpy()
            matrix = cdist(out_asso.cpu().detach().numpy(), out_asso.cpu().detach().numpy(), metric='euclidean')
            ...
            TP = np.sum((association_map == 1) & equal_mask)  # 预测为1，且与gt_asso_map相等
            FP = np.sum((association_map == 1) & ~equal_mask) # 预测为1，但与gt_asso_map不相等
            TN = np.sum((association_map == 0) & equal_mask)  # 预测为0，且与gt_asso_map相等
            FN = np.sum((association_map == 0) & ~equal_mask) # 预测为0，但与gt_asso_map不相等
            asso_cm[0] += TP
            asso_cm[1] += FP
            asso_cm[2] += TN
            asso_cm[3] += FN
...
def compute_loss(model, config, in_joints, out_joints, mmsi_list, in_masks, out_masks, padding_mask, epoch=None, mode='val', loss_last=True, optimizer=None):
    ...
    metamask = (mode == 'train')
    # pdb.set_trace()
    # in_joints.shape  torch.Size([8, 30, 3, 4])
    # out_joints.shape  torch.Size([8, 5, 3, 4])
    pred_joints, out_asso = model(in_joints, padding_mask, metamask=metamask)  # torch.Size([8, 35, 1, 2]), torch.Size([8, 128])
    # pdb.set_trace()
...
def train(config, logger, experiment_name="", dataset_name=""):
    ...
    if config["MODEL"]["checkpoint"] != "":
        # logger.info(f"Loading checkpoint from {os.path.join('/data/ymh/MOT/codes/MSTA/tracking/experiments/MSTAv2/checkpoints', config["MODEL"]["checkpoint"])}")
        # pdb.set_trace()
    ...
    # checkpoint = torch.load(os.path.join('/data/ymh/MOT/codes/MSTA/tracking/experiments/AccessAIS_wo_asso_loss/checkpoints', 'best_val_checkpoint.pth.tar'))
    # model.load_state_dict(checkpoint["model"])
    ...
    #     if val_ade_pred < min_val_loss:

    #         min_val_loss = val_ade_pred
    ...
    for epoch in range(100):
        ...
        for i in range(train_steps):
            ...
            loss_pred, loss_recon, loss_asso, pred_joints, out_asso, asso_cm_batch = compute_loss(model, config, in_joints, out_joints, mmsi_list, in_masks, out_masks, padding_mask, epoch=epoch, mode='train', optimizer=None)
            # pdb.set_trace()
            out_asso_sum += torch.sum(out_asso).detach()
            asso_cm[0] += asso_cm_batch[0]
            asso_cm[1] += asso_cm_batch[1]
            asso_cm[2] += asso_cm_batch[2]
            asso_cm[3] += asso_cm_batch[3]
        ...
        bar.finish()
        print(f"[Train]  TP: {asso_cm[0]}, FP: {asso_cm[1]} \n[Train]  TN: {asso_cm[2]}, FN: {asso_cm[3]}")
        print(f'[Train] out_asso_sum: {out_asso_sum}')
