...
split_list = ['test']
save_root = f'{args.data_root}/{args.dataset_name}_PMST_TEST/det_results/{args.dataset_name}_PMST_TEST'
cam_para_save_root = f'{args.data_root}/{args.dataset_name}_PMST_TEST/cam_para/{args.dataset_name}_PMST_TEST'
    ...
    for file_name in os.listdir(path_split):
        ...
        for mmsi in gt_mmsi_list:
            ...
            for i in range(gt_mmsi_sort.shape[0]):
                ...
                bb_left = lon
                bb_top = lat
                bb_width, bb_height = 0.003, 0.001  
        ...
        for t in measure_time_list:
            ...
            for i in range(measure_t.shape[0]):
                ...
                bb_left = lon_left
                bb_top = lat_top
                bb_width, bb_height = abs(lon_left - lon_right), abs(lat_top - lat_bottle)  
                if source == 'elec':
                    conf = 0.6
                else:
                    conf = 0.8
                measure_txt_wf.write(str(int(t+1)) + ',' + str(int(i)) + ',' + str(bb_left) + ',' +
                                str(bb_top) + ',' + str(bb_width) + ',' + str(bb_height) + ',' +
                                str(sog) + ',' + str(cog) + ',' + str(conf) + ',-1,-1,-1' + '\n')
