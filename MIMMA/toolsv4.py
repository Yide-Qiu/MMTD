...
from tracking.dataset_xysc import batch_process_coords, collate_batch, read_row_files, MultiPersonTrajPoseDataset
...
class MultiSensorShipTrackingModel:
    def __init__(self, args):
        ...
        self.finish_time_max = 60
        self.pred_mmsi = 0
        self.merge_t_segment = 10
        self.merge_t_interval = 1
        self.merge_t_thresh = 0.04  # 1500m
        self.merge_point_thresh = 0.04  # 4000m
        self.behavior_len_thresh = 8
        self.device = torch.device('cuda:0')
        self.crt_t = 0

    def merge_trajs(self, traj_list):
        # 随着时间增加，需要遍历的航迹时间也增加，导致后期时间消耗大是正常的
        # Convert each trajectory array into a dictionary for easier manipulation
        traj_dicts = []
        for arr in traj_list:
            traj_dict = {}
            # pdb.set_trace()
            for t, x, y, s, c, source, satellite_id, b in arr:
                # traj_dict[source][int(t)] = (x, y, s, c, source, b)  # Store (x, y, s, c) tuples indexed by time
                traj_dict[int(t)] = (x, y, s, c, source, satellite_id, b)  # Store (x, y, s, c) tuples indexed by time
        ...
        for t in all_times:
            ...
            for traj_dict in traj_dicts:
                if t in traj_dict:
                    x, y, s, c, so, satellite_id, b = traj_dict[t]
            ...
            if len(b_counter) == 1:
                b_final = b_counter.most_common(1)[0][0]
            elif b_counter.most_common(1)[0][0] == 'None':
                b_counter.pop('None')
                b_final = b_counter.most_common(1)[0][0]
            ...
            results.append([t, x_mean, y_mean, s_mean, c_mean, so_final, satellite_id_final, b_final])
    ...
    def multi_source_merging_working(self):
        ...
        for traj_id in self.ship_traj_dict.keys():
            if len(self.ship_traj_dict[traj_id]['traj']) < self.merge_t_segment:
                continue
            # pdb.set_trace()
            merging_segment_list.append(np.array(self.ship_traj_dict[traj_id]['traj'], dtype=object)[-self.merge_t_segment:, 2:5])
        ...
        for i in range(distance_matrix.shape[0]):
            ...
            # 初始化
            traj_list = [np.array(self.ship_traj_dict_pred[traj_id_list[i]]['traj'])[:, 2:].astype(object)]
            ...
            for j in range(1, distance_matrix.shape[1]):
                ...
                if distance_matrix[i, j] <= self.merge_t_thresh:
                    ...
                    traj_list.append(np.array(self.ship_traj_dict_pred[traj_id_list[j]]['traj'])[:, 2:].astype(object))
            ...
            self.ship_traj_dict[traj_id_list[i]]['traj'] = traj_merge[:, :-1].tolist()
    ...
    def split_long_and_short_traj(self, obs_array_t):
        # 组织数据，将短轨迹和长轨迹划分开
        ship_traj_long = []
        ship_mmsi_long = []
        ship_traj_short = []
    ...
    def long_traj_predict(self, ship_traj_long):
        ...
        satellite_id_list = np.unique(ship_traj_long[:, 0, -1])
        pdo = 0
        ef = 0
        argwhere_list = np.zeros(shape=(ship_traj_long.shape[0]), dtype=np.int16)
        tid = 0
        for satellite_id in satellite_id_list:
            start_time1 = time.time()
            obs_i = ship_traj_long[ship_traj_long[:, 0, -1] == satellite_id]
            for ai in np.argwhere(ship_traj_long[:, 0, -1] == satellite_id):
                argwhere_list[ai] = tid
                tid += 1
            # print(obs_i.shape)
            # pdb.set_trace()
            dataset = MultiPersonTrajPoseDataset(obs_i[:, :, :-1], split=self.args.split, track_size=(self.in_F+self.out_F), track_cutoff=self.in_F)
            ...
            # 轨迹预测
            start_time2 = time.time()
            traj_pred_result_points_i = evaluate_frame(self.tracking_model, self.args.modality, dataloader, self.tracking_config)
            end_time2 = time.time()
            ef += (end_time2 - start_time2)
            # todo 使用算法对所有轨迹进行预测
            traj_pred_result_points.append(np.array(traj_pred_result_points_i[:, 0, 0, :]))  # 预测结果
        print('pdo:', pdo, end=' ')
        print('ef:', ef, end=' ')
        # pdb.set_trace()
        traj_pred_result_points = np.concatenate(traj_pred_result_points)
        traj_pred_result_points = traj_pred_result_points[argwhere_list]
        return traj_pred_result_points

    def long_traj_update(self, obs_array_t, traj_pred_result_points, ship_mmsi_long):
        matched_obs_idx_list = []
        matched_pred_idx_list = []
        matches, cost_matrix = hungarian_matching_traj(obs_array_t[:, 3:5].astype(np.float32),
                                                            traj_pred_result_points[:, :2].astype(np.float32))  # 选用第t+1帧的预测结果
        # pdb.set_trace()
        for obs_idx, pred_idx in matches:
            # 对于匹配上的{观测点,预测点}，将预测点加入到ship_traj_list中
            if cost_matrix[obs_idx, pred_idx] >= 0.04:
                continue
            # pdb.set_trace()
            ...
            if obs_array_t[obs_idx][-1] in ['radar', 'image']:
                self.ship_traj_dict_pred[ship_mmsi_long[pred_idx]]['traj'].append(np.array([*obs_array_t[obs_idx], 'None'], dtype=object))
            else:
                self.ship_traj_dict_pred[ship_mmsi_long[pred_idx]]['traj'].append(np.array([*obs_array_t[obs_idx][:3],
                                                                                                *traj_pred_result_points[pred_idx],
                                                                                                *obs_array_t[obs_idx][7:], 'None'], dtype=object))
                self.ship_traj_dict_pred[ship_mmsi_long[pred_idx]]['traj'][-1][-3] = 'pred'
            self.ship_traj_dict[ship_mmsi_long[pred_idx]]['traj'].append(obs_array_t[obs_idx])
            self.ship_traj_dict[ship_mmsi_long[pred_idx]]['traj'][-1][1] = ship_mmsi_long[pred_idx]
        ...
        # 没有匹配上的轨迹点则直接将预测结果加入到ship_traj_list中 或者 直接剔除掉？
        for i in range(len(ship_mmsi_long)):
            if i not in matched_pred_idx_list:
                ...
                # pdb.set_trace()
                self.ship_traj_dict[ship_mmsi_long[i]]['traj'].append(np.array(['model', ship_mmsi_long[i], crt_t,
                                                                                    *traj_pred_result_points[i],
                                                                                    *last_ship_point[7:]], dtype=object))
                # last_ship_point_pred = self.ship_traj_dict_pred[ship_mmsi_long[i]]['traj'][-1]
                self.ship_traj_dict_pred[ship_mmsi_long[i]]['traj'].append(np.array(['model', ship_mmsi_long[i], crt_t,
                                                                                         *traj_pred_result_points[i], 'pred', last_ship_point[-1], 'None'], dtype=object))
                # pdb.set_trace()
    ...
    def short_traj_update(self, obs_array_t, traj_pred_points_short, ship_traj_short, ship_mmsi_short, matched_obs_idx_list):
        ...
        for obs_idx, pred_idx in matches:
            # 对于匹配上的{观测点,预测点}，将预测点加入到ship_traj_list中
            if cost_matrix[obs_idx, pred_idx] >= 0.08:
                continue
            if obs_idx in matched_obs_idx_list:
                continue
            # import pdb
            pdb.set_trace()
            # 将当前时间与上次记录的时间中间的数据插值出来
            for t1 in range(int(ship_traj_short[pred_idx, 0]) + 1, crt_t):
                ...
                #     pdb.set_trace()
                linear_data = obs_array_t[obs_idx][3:7].astype(np.float32) - np.array(self.ship_traj_dict[ship_mmsi_short[pred_idx]]['traj'][-1][3:7], dtype=np.float32)
                linear_data = linear_data * (t1 - int(ship_traj_short[pred_idx, 0]))
                linear_data = linear_data / (crt_t - int(ship_traj_short[pred_idx, 0]))
                linear_data = linear_data + np.array(self.ship_traj_dict[ship_mmsi_short[pred_idx]]['traj'][-1][3:7], dtype=np.float32)
                self.ship_traj_dict[ship_mmsi_short[pred_idx]]['traj'].append(np.array(['linear', ship_mmsi_short[pred_idx], t1,
                                                                                                *linear_data, *obs_array_t[obs_idx][-2:]], dtype=object))
                self.ship_traj_dict_pred[ship_mmsi_short[pred_idx]]['traj'].append(np.array(['linear', ship_mmsi_short[pred_idx], t1, *linear_data,
                                                                                                    *obs_array_t[obs_idx][-2:], 'None'], dtype=object))
            self.ship_traj_dict[ship_mmsi_short[pred_idx]]['traj'].append(obs_array_t[obs_idx])
            self.ship_traj_dict[ship_mmsi_short[pred_idx]]['traj'][-1][1] = ship_mmsi_short[pred_idx]
            self.ship_traj_dict_pred[ship_mmsi_short[pred_idx]]['traj'].append(np.array([*obs_array_t[obs_idx], 'None'], dtype=object))
            self.ship_traj_break_time_dict[ship_mmsi_short[pred_idx]] = 0
            matched_obs_idx_list.append(obs_idx)
            matched_pred_idx_list.append(pred_idx)
        # 没有匹配上的短轨迹进行中断计数，超过一定长度的就删除
    ...
    def add_new_traj(self, obs_array_t, matched_obs_idx_list):
        ...
        for i in range(obs_array_t.shape[0]):
            if i not in matched_obs_idx_list:
                ...
                self.ship_traj_dict[self.pred_mmsi]['traj'][-1][1] = self.pred_mmsi
                self.ship_traj_dict_pred[self.pred_mmsi] = {'traj':[], 'resource':[]}
                self.ship_traj_dict_pred[self.pred_mmsi]['traj'].append(np.array([*obs_array_t[i], 'None'], dtype=object))
    ...
    def multi_object_tracking(self, obs_array_t):
        start_time = time.time()
        ship_traj_long, ship_mmsi_long, ship_traj_short, ship_mmsi_short = self.split_long_and_short_traj(obs_array_t)
        matched_obs_idx_list = []
        if len(ship_traj_long) != 0:
            traj_pred_result_points = self.long_traj_predict(np.array(ship_traj_long, dtype=object))
            matched_obs_idx_list = self.long_traj_update(obs_array_t, traj_pred_result_points, ship_mmsi_long)
        # 没有匹配上的观测点 与 短轨迹序列进行匹配
    ...
    def get_ship_traj_dict(self):
        ...
        for traj_id in list(self.ship_traj_dict_pred.keys()):
            ...
            if len(traj) < self.merge_t_segment:
    ...
    def save_tracking_traj_dict(self, save_path):
        ...
        for k in list(ship_traj_dict_all.keys()):
            ...
            if len(ship_traj) < self.merge_t_segment:
                continue
            # pdb.set_trace()
            if ship_resource.shape[0] == 0:
                continue
            mmsi = ship_resource[0, -1]
            txt_name = str(k) + '_' + str(mmsi) + '.txt'
            wf = open(os.path.join(save_path, 'ship_traj_dict_all', txt_name), 'w')
            # behavior_most = Counter(ship_resource[:, 1]).most_common(1)[0][0]
            label1_most = Counter(ship_resource[:, 2]).most_common(1)[0][0]
            label2_most = Counter(ship_resource[:, 3]).most_common(1)[0][0]
            for idx in range(len(ship_traj)):
                # pdb.set_trace()
                pid, p, f, x, y, s, c, source, satellite_id, behavior = list(ship_traj[idx])
                wf.write(str((pid)) + ',' + str(int(p)) + ',' + str(int(float(f))) + ',' + str(x) + ',' + str(y) + ',' + str(s) + ',' + str(c)
                         + ',' + str(behavior) + ',' + str(label1_most) + ',' + str(source) + '\n')
            wf.close()
        for k in list(ship_traj_finish_dict_all.keys()):
            ship_traj = ship_traj_finish_dict_all[k]['traj']
            ship_resource = np.array(ship_traj_finish_dict_all[k]['resource'])
            if len(ship_traj) < self.merge_t_segment:
                continue
            # pdb.set_trace()
            if ship_resource.shape[0] == 0:
...
def evaluate_frame(model, modality_selection, dataloader, config):
    ...
    for i, batch in enumerate(dataloader):
        ...
        # import pdb
        # pdb.set_trace()
        for j in range(pred_joints.shape[0]):
            pred_joints[j, :, 0, 0] += joints[j, 0, -1, 0, 0]
            pred_joints[j, :, 0, 1] += joints[j, 0, -1, 0, 1]
            pred_joints[j, :, 0, 2] += joints[j, 0, -1, 1, 0]
            pred_joints[j, :, 0, 3] += joints[j, 0, -1, 2, 0]
        # import pdb
        # pdb.set_trace()

        out_joints = out_joints.cpu()  # torch.Size([64, 12, 10, 4])  10 == 5 * 2

        pred_joints = pred_joints.cpu().reshape(out_joints.size(0), 12, 1, 4)  # torch.Size([64, 12, 1, 2])
        pred_joints_list.append(pred_joints)
    # import pdb
    # pdb.set_trace()
    if pred_joints_list == []:
        # pdb.set_trace()
        return pred_joints_list
...
def init_config():
    ...
    parser.add_argument("--modality", type=str, default="traj", help="available modality combination from['traj','traj+2dbox']")
