...
class MultiSensorShipTrackingModel:
    def __init__(self, args):
        ...
        self.use_mftp_module = True
        self.use_msta_module = True
        if args.module == 'MFTP':
            self.use_msta_module = False
        elif args.module == 'MSTA':
            self.use_mftp_module = False

        if not self.use_mftp_module:
            self.kalmanFilters = {}
        ...
        self.pred_mmsi = 0
        self.association_thresh_probability = args.thresh_prob
        self.association_thresh_ground = args.thresh_ground
    ...
    def add_new_traj_kalman(self, obs_array_t, matched_obs_idx_list):
        # 实在没有匹配上的观测点则作为新的轨迹
        for i in range(obs_array_t.shape[0]):
            if i not in matched_obs_idx_list:
                # resource --> point_id, behavior, label1, label2, source
                # obs_array_t --> [f, t, l , b, r, s, c, source, satellite]
                self.kalmanFilters[self.pred_mmsi] = {'kalman': KalmanFilter()}
                mean, covariance = self.kalmanFilters[self.pred_mmsi]['kalman'].initiate(self.tlwh_to_xyah(self.tlbr_to_tlwh(obs_array_t[i][1:5])))
                self.kalmanFilters[self.pred_mmsi]['mean'] = mean
                self.kalmanFilters[self.pred_mmsi]['covariance'] = covariance
    ...
    def remove_duplicate_stracks_msta(self, association_map):
        if len(association_map) == 0:
            return
        points = []
        k_list = []
        points_start = []
        for k in list(self.ship_traj_dict_pred.keys()):
            ship_traj = self.ship_traj_dict_pred[k]['traj']
            points.append(ship_traj[-1])
            points_start.append(ship_traj[0])
            k_list.append(k)
        if len(points) == 0:
        ...
        # pdb.set_trace()
        cost_matrix = np.linalg.norm(points_xy[:, np.newaxis] - points_xy, axis=2)
        asso_ground = cost_matrix < self.association_thresh_ground

        # remain_k_list = []
        # remove_id_list = []
        # # pdb.set_trace()
        # for thresh in range(1, 100):
        #     asso_model = association_map > thresh / 100
        #     association = asso_model & asso_ground
        #     remain_k_list = []
        #     remove_id_list = []
        #     for p in range(association_map.shape[0]):
        #         if p in remove_id_list:
        #             continue
        #         low_cost_idx = np.argwhere(association[p] == True)
        #         timep = points[p][0] - points_start[p][0]
        #         longest_idx = [p, timep]
        #         # print(low_cost_idx)
        #         # print(' ')
        ...
        asso_model = association_map > self.association_thresh_probability
        association = asso_model & asso_ground
        # if association_map.shape[0] != len(points):
        #     pdb.set_trace()
    ...
    def remove_duplicate_stracks(self):
        pointsa = []
        pointsb = []
        k_lista = []
        k_listb = []
        ...
        for k in list(self.ship_traj_dict_pred.keys()):
            ship_traj = self.ship_traj_dict_pred[k]['traj']
            break_time = self.ship_traj_break_time_dict[k]
            if break_time == 0:
                pointsa.append(ship_traj[-1])
                pointsa_start.append(ship_traj[0])
                k_lista.append(k)
            else:
                pointsb.append(ship_traj[-1])
                pointsb_start.append(ship_traj[0])
                k_listb.append(k)
        # pdb.set_trace()
        ...
        # Euclidean distance
        pointsa = np.array(pointsa, dtype=object)
        pointsb = np.array(pointsb, dtype=object)
        pointsa_xy = pointsa[:, 3:5].astype(np.float64)
        pointsb_xy = pointsb[:, 3:5].astype(np.float64)
        # pdb.set_trace()
        cost_matrix = np.linalg.norm(pointsa_xy[:, np.newaxis] - pointsb_xy, axis=2)
        # pdb.set_trace()

        # pairs = np.where(cost_matrix < self.association_thresh_ground)
        # if pairs[0].shape[0] == 0 and pairs[1].shape[0] == 0:
        #     return
        # remain_k_lista = []
        # remain_k_listb = []
        # for p, q in zip(*pairs):
        #     # timep = pointsa[p].frame_id - pointsb[p].start_frame
        #     # timeq = pointsa[q].frame_id - pointsb[q].start_frame
        #     timep = pointsa[p][0] - pointsa_start[p][0]
        #     timeq = pointsb[q][0] - pointsb_start[q][0]
        #     if timep > timeq:
        #         remain_k_lista.append(k_lista[p])
        #     else:
        #         remain_k_listb.append(k_listb[q])
        # pdb.set_trace()
        # for k in list(self.ship_traj_dict.keys()):
        #     if k not in remain_k_lista or k not in remain_k_listb:
        #         self.ship_traj_finish_dict[k] = self.ship_traj_dict_pred[k]
        #         del self.ship_traj_dict[k]
        #         del self.ship_traj_dict_pred[k]
        #         del self.ship_traj_break_time_dict[k]
        ...
        for p in range(cost_matrix.shape[0]):
            if p in remove_id_list:
                continue
            low_cost_idx = np.argwhere(cost_matrix[p] < self.association_thresh_ground)
            timep = pointsa[p][0] - pointsa_start[p][0]
            longest_idx = [p, timep]
            p_flag = 1
            for q in low_cost_idx:
                ...
                remove_id_list.append(q)
            if p_flag:
                remain_k_list.append(k_lista[longest_idx[0]])
            else:
                remain_k_list.append(k_listb[longest_idx[0]])
        # pdb.set_trace()
        for k in list(self.ship_traj_dict.keys()):
            if k not in remain_k_list:
                self.ship_traj_finish_dict[k] = self.ship_traj_dict_pred[k]
    ...
    def traj_predict_msta(self, ship_traj_list):
        ...
        end_time1 = time.time()
        dataloader = DataLoader(dataset, batch_size=8, num_workers=self.tracking_config['TRAIN']['num_workers'],
    ...
    def traj_predict_kalman(self, ship_traj_list, ship_mmsi_list):
        obs = ship_traj_list
        dataset = MultiPersonTrajPoseDataset(obs, split=self.args.split, track_size=(self.in_F+self.out_F), track_cutoff=self.in_F)
        dataloader = DataLoader(dataset, batch_size=8, num_workers=self.tracking_config['TRAIN']['num_workers'],
                                shuffle=False, collate_fn=collate_batch_mot)
        _, refine_joints_list, asso_feature_list = evaluate_frame(self.tracking_model, self.args.modality, dataloader, self.tracking_config)
        # 使用算法
        if obs.shape[0] == 0:
            return [], []
        matrix = cdist(asso_feature_list.cpu().detach().numpy(), asso_feature_list.cpu().detach().numpy(), metric='euclidean')
        ...
        for mmsi in ship_mmsi_list:
            mean_kalman = self.kalmanFilters[mmsi]['mean']
            covariance_kalman = self.kalmanFilters[mmsi]['covariance']
            mean, covariance = self.kalmanFilters[mmsi]['kalman'].predict(mean_kalman, covariance_kalman)
            self.kalmanFilters[mmsi]['mean'] = mean
            self.kalmanFilters[mmsi]['covariance'] = covariance
            tlwh = self.mean_to_tlwh(mean)
            pred_joints_list.append(tlwh[:2])
        pred_joints_list = np.array(pred_joints_list, dtype=np.float32)
        return pred_joints_list, association_map
    ...
    def traj_update(self, obs_array_t, traj_pred_result_points, ship_mmsi, ship_traj):
        ...
        for pred_idx, obs_idx in matches:
            ...
            if cost_matrix[pred_idx, obs_idx] >= self.association_thresh_ground:
                continue
            # pdb.set_trace()
    ...
    def traj_update_kalman(self, obs_array_t, traj_pred_result_points, ship_mmsi, ship_traj):
        matched_obs_idx_list = []
        matched_pred_idx_list = []
        if obs_array_t.shape[0] == 0 or traj_pred_result_points == []:
            matches = []
            ...
            # 暂时就用下一帧，以后可以考虑中断多长时间就用第几帧
            matches, cost_matrix = hungarian_matching_traj(traj_pred_result_points.astype(np.float32), obs_array_t[:, 3:5].astype(np.float32))  # 选用第t+1帧的预测结果
        # pdb.set_trace()
        for pred_idx, obs_idx in matches:
            # 对于匹配上的{观测点,预测点}，将预测点加入到ship_traj_list中
            if cost_matrix[pred_idx, obs_idx] >= self.association_thresh_ground:
            ...
            self.ship_traj_dict_pred[ship_mmsi[pred_idx]]['traj'].append(obs_array_t[obs_idx])
                # self.ship_traj_dict_pred[ship_mmsi[pred_idx]]['traj'][-1][-2] = 'pred'
            self.ship_traj_dict[ship_mmsi[pred_idx]]['traj'].append(obs_array_t[obs_idx])
            self.ship_traj_break_time_dict[ship_mmsi[pred_idx]] = 0
            matched_obs_idx_list.append(obs_idx)
            matched_pred_idx_list.append(pred_idx)
            history_mean, history_covariance = self.kalmanFilters[ship_mmsi[pred_idx]]['mean'], self.kalmanFilters[ship_mmsi[pred_idx]]['covariance']
            new_mean, new_covariance = self.kalmanFilters[ship_mmsi[pred_idx]]['kalman'].update(history_mean, history_covariance,
                                                                   self.tlwh_to_xyah(self.tlbr_to_tlwh(obs_array_t[obs_idx, 1:5])))
            self.kalmanFilters[ship_mmsi[pred_idx]]['mean'], self.kalmanFilters[ship_mmsi[pred_idx]]['covariance'] = new_mean, new_covariance
        # 没有匹配上的轨迹点则直接将预测结果加入到ship_traj_list中 或者 直接剔除掉？ ByteTrack里面是不保存这部分结果的
        for i in range(len(ship_mmsi)):
            if i not in matched_pred_idx_list:
                # 更新丢失次数，当丢失次数超过阈值就判定为结束的轨迹
                ...
                if self.ship_traj_break_time_dict[ship_mmsi[i]] >= self.finish_time_max:
                    ...
                    del self.ship_traj_dict_pred[ship_mmsi[i]]
                    del self.ship_traj_break_time_dict[ship_mmsi[i]]
                    del self.kalmanFilters[ship_mmsi[i]]
        return matched_obs_idx_list
    ...
    def multi_object_tracking(self, obs_array_t, obs_mmsi_t):
        ...
        if self.use_mftp_module:
            traj_pred_result_points, association_map = self.traj_predict_msta(np.array(ship_traj_list, dtype=object))
            matched_obs_idx_list = self.traj_update(obs_array_t, traj_pred_result_points, ship_mmsi_list, ship_traj_list)
        else:
            traj_pred_result_points, association_map = self.traj_predict_kalman(np.array(ship_traj_list, dtype=object), ship_mmsi_list)
            matched_obs_idx_list = self.traj_update_kalman(obs_array_t, traj_pred_result_points, ship_mmsi_list, ship_traj_list)
        if self.use_msta_module:
            self.remove_duplicate_stracks_msta(association_map)
        else:
        ...
        if self.use_mftp_module:
            ...
            self.add_new_traj_kalman(obs_array_t, matched_obs_idx_list)
        # end_time = time.time()
...
def init_tracking_model(args):
    ...
    tracking_ckpt = torch.load(os.path.join(args.ckpt_root, args.ckpt_dir, 'checkpoints/best_val_checkpoint.pth.tar'), map_location = torch.device('cuda:0'))
